...
export const Header: React.FC<HeaderProps> = ({ onMenuToggle }) => {
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between shrink-0 z-30 sticky top-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 id="header-page-title" className="text-lg sm:text-xl font-bold text-slate-900 leading-tight">
            {getPageTitle()}
          </h1>
      ...
      <div className="flex items-center space-x-3 sm:space-x-5">
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition-all cursor-pointer select-none ${
            isDemoMode
        ...
        <div className="relative">
          ...
          {showRoleDropdown && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 flex items-center gap-1.5">
              ...
              <div className="py-1">
                {users.map((u) => (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2 text-xs flex items-center justify-between hover:bg-[#FFF1F2] transition-colors cursor-pointer ${
                      currentUser.id === u.id ? 'bg-[#FFF1F2] text-[#E31B23] font-bold' : 'text-slate-700'
                    }`}
                  >
                    <div className="flex items-center gap-2">
              ...
              <div className="pt-1.5 mt-1 border-t border-slate-100 px-2">
                <button
                  ...
                  className="w-full py-1.5 px-3 rounded-lg text-left text-xs font-semibold text-slate-600 hover:bg-slate-100 flex items-center justify-between cursor-pointer"
                >
